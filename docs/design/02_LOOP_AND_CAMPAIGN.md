...
flowchart TD
    ...
    subgraph INFO["Information network — the Listening Post"]
        RADIO["Radio capacity\n(channel-hour slots)"]
        INTEL["Intel\n(forecasts • claims • leads,\nwith confidence & expiry)"]
        S45["Signal 45\nverification track"]
    end
    ...
    subgraph OUT["Outside"]
        EXP["Nightruns\n(active or delegated)"]
        LOC["Locations\n(tunnel-linked)"]
        VIS["Visitors & arrivals\nat the Scrubber Gate"]
        FAC["Factions\n(Tidemill • Linewalkers • line voices)"]
    end
    ...
    ROOMS -->|"produce / maintain"| RES
    ROOMS -->|"draw"| UTIL
    UTIL -->|"enable & fail visibly"| ROOMS
    STORM -->|"load & damage"| UTIL
    RES -->|"consume"| SURV
    RES --> CRAFT
    CRAFT -->|"items, repairs, provisions"| ROOMS
    CRAFT -->|"masks, kit"| EXP
    SURV -->|"needs & Strain"| REL
    REL --> ACC
    ACC -->|"authority, refusals, scenes"| SURV
    RADIO --> INTEL
    INTEL -->|"warnings"| STORM
    INTEL -->|"reveal / mutate"| LOC
    INTEL -->|"beacons"| VIS
    INTEL --> S45
    SURV -->|"personal signals"| RADIO
    UTIL -->|"charge & antenna"| RADIO
    EXP -->|"salvage, meds, food"| RES
    EXP -->|"injury, Strain, Marks"| SURV
    EXP -->|"contacts"| FAC
    LOC --> EXP
    VIS -->|"trade, arrivals, demands"| ACC
    VIS --> FAC
    FAC -->|"pressure & pacts"| EVENTS
    EVENTS -->|"crises & opportunities"| UTIL
    EVENTS --> REL
    EVENTS --> VIS
    S45 --> PROG
    ACC --> PROG
    PROG --> END
    UTIL -->|"collapse axis"| END
    ACC -->|"collapse axis"| END
```
