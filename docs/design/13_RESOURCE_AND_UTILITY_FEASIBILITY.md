...
Prompt 3 completed this refinement in documents 18–23 and executable modules for survival, medical care, forecasts, utilities, and incidents. Exactly five stocks and four utilities remain. Power/Charge are distinct, Water names one of five failing links, Air advances through visible stages, Structure is local, and the storm stays bounded to Air → Power → Water. The Prompt 2 tables remain the original numerical baseline; `model.json` and the generated Prompt 3 report are now authoritative for current results.
