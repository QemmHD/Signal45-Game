...
**Stage:** Prompt 3 (amended by the D-045 red-team pass). **Status:** LOCKED at structural level. **All numeric values are PROVISIONAL** (D-043) and live in the config set — stock/needs numbers in `tools/data/resources.json`, utility numbers in `utilities.json`, incident/condition/HIGHBALL numbers in `incidents.json` — this document explains structure and behavior; it never duplicates numbers the configs own. Validation: `tools/survival_model.py` (23 scenarios) + `tools/test_survival_model.py` (36 checks).
...
**Layer 3 — item families** (storage/crafting/expedition detail): thirteen families architected in `resources.json → item_families`. The slice tracks **tools and radio components** distinctly (they gate specific projects — the lift and the splice), folds metal/parts/textiles into Materials, and **defers the other eight** (filters included: masks and protective gear resolve as run-provisioning flags in the slice, and the air incidents cost Materials + WU and say so; filters gate the full game's replacement orders — D-045). **The anti-lie rule:** a blocked project always names its missing family ("needs: antenna wire — Depot 9 or Sable"), never a bare "not enough Materials." Detailed recipes are the crafting stage's work.
...
### Food (person-meals)
Consumed at the **two visible meal windows**; produced by runs, trade, and the Canteen's waste reduction. Storage caps at the pantry (Cold Store raises it). Perishables from runs must be eaten within 2 days or convert to stable rations at half value — **forecast, never silent loss** *(architecture-only in the slice: `spoilage.slice_active: false`; the rule ships at first playable — D-045)*. Shortage stages: *thin* (forecast chip) → *short meals* (hunger + stress steps; no injury — one missed meal is never critical harm, asserted in tests) → *sustained shortage* (work efficiency down — modeled and asserted via the ladder in the unrationed-shortage scenario, 20 §2; the further health-erosion stage is specified but needs a longer-than-slice shortage and is first-playable work, with its own warning when it lands). Emergency: **ration** (fewer meals/day; saves stock, costs stress and a remembered decision — modeled), trade, an extra run. Full-game: ingredient variety, the Spore Beds/Grow Gallery production chain.
...
### Charge (cell-hours)
The stored buffer between the Flywheel (producer) and the load board (consumers). Surplus banks to the cell rack (cap visible); deficits shed **the lowest occupied tier first** — the Optional tier's single load, **PLATFORM COMFORT LIGHTING** (D-047: visible dimming, no survival harm, modest Stress only after extended darkness, auto-restores), then Normal (hotplate/tools) — load by load **with a banner naming what went dark, never silently** (repeats become resident memories, not repeated banners); only a residual deficit draws the rack. The battery reserve bridges the critical tier one phase after producer failure — the canonical **"Battery: ~1 phase if the Flywheel fails"** Fact forecast (18 §8) — and bridging is not free: the Normal tier goes dark while it runs. Emergency: shed tiers, battery, HIGHBALL overdrive (19), hand-cranking (brutal labor fallback).

## 3. The main HUD specification

Five chips + day/phase clock (unchanged budget): each chip shows **amount · trend arrow · warning state** (lumpy stocks add the free-vs-reserved count on the chip itself); the ≈person-days/operating-time projection lives in the **one-tap expansion** with the rest of the forecast contract (11 §4.1 is the normative chip spec — D-045 alignment): steady stocks show top consumer + top idle producer; lumpy stocks (Medicine, Materials) show **free vs. reserved/booked** + the upcoming-draws list + earliest failing commitment. **Two-tap diagnosis rule (07-tested):** any major shortage's main source is identifiable in ≤2 taps. No formulas in normal play; the advanced inspect layer holds raw numbers.
