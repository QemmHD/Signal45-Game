...
### Charge (cell-hours)
The stored buffer between the Flywheel (producer) and the load board (consumers). Surplus banks to the cell rack (cap visible); deficits shed **the lowest occupied tier first** (Normal — hotplate/tools — in the slice, which ships no optional-tier loads; D-045) load by load **with a banner naming what went dark, never silently**; only a residual deficit draws the rack. The battery reserve bridges the critical tier one phase after producer failure — the canonical **"Battery: ~1 phase if the Flywheel fails"** Fact forecast (18 §8) — and bridging is not free: the Normal tier goes dark while it runs. Emergency: shed tiers, battery, HIGHBALL overdrive (19), hand-cranking (brutal labor fallback).
