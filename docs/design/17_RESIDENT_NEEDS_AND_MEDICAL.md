...
| **Health** | Injury, illness, exposure, untreated conditions; treatment + rest to recover | Limps, coughs, bed rest; condition card | Restrictions by condition class | Treatment (Medicine committed at start) + treatment days | **Critical health auto-pauses** (modal: "a resident may die" tier); slice: no death (D-043); full game: death only via serious condition + warning + response window + deliberate risk |
...
Needs are four floats + a condition set per resident inside the single world-state object (R-10); treatment bookings serialize with their committed doses; reloads can neither repeat a dose nor lose a booking (asserted in `test_survival_model.py`).
