...
**Canonical slice cascade** (modeled, both directions): *Flywheel instability → lowest-tier loads shed (Normal: hotplate/tools) → scrubbers underpowered → east-wing air degrades (3-phase window) → exposure.* Interrupted at step 1 by HIGHBALL/repair, at step 2 by battery + shedding (both at real cost — the bridge spends charge and darkens the Normal tier), at any point by evacuation/masks; ignored, it stops at step 3 (respiratory conditions — bounded, costed, recoverable).
