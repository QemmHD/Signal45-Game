...
Steps 1–7 (select area → choose family → preview footprint → preview access/utilities → preview sockets → review costs → confirm blueprint) are **immediate planning** — free, reversible, no reservation. The blueprint confirm card offers **"Start now" / "Save for later"** (default: Start now — the ≤6-tap commit path holds); **Start is the one tap that commits resources** (D-048), on the confirm card or on a saved blueprint's card, and it always shows the cost it transfers. Step 8 (reserve) is **automatic at activation — the Start command**. A saved-inactive blueprint has a named status in the section queue and on its card (*planned — not started · Start / Discard*), inside the named-blocker channel. Steps 9–12 (deliver → stages appear → equipment installs) are **resident-performed work** through the existing works-order system (11 §5's schema unchanged). Step 13 (operational) is automatic; step 14 (modules/upgrades/decoration) is optional later ADAPT work. **Small installations skip the ceremony** (D-040's task classes are binding): a bed, lamp, cabinet, or partition is a *placement* or *short install* — tap, ghost, confirm, a brief carry-and-install task; only projects and major reclamations use the full staged flow.
